...
Run the sweep-owned DMO, DLO, DLO-to-DMO mapping, and semantic model shell mutation lifecycles only against the disposable sweep org:
...
- The capability sweep plans dry-run coverage for every facade capability, runs bounded read/safe-post live checks, dynamically follows list responses into detail reads when public-safe identifiers are available, and can run sweep-owned DMO, DLO, DLO-to-DMO mapping, and semantic model shell lifecycles behind an explicit destructive gate.
